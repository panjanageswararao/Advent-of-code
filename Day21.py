    ...
    while not all_sets_are_len_one(allergen_map_unique):
        for k, v in allergen_map_unique.items():
            if k not in uniquely_mapped and len(v) == 1:
                keys_to_remove_from = [
                    key_to_remove_from
                    for key_to_remove_from in allergen_map_unique
                    if key_to_remove_from != k
                ]
